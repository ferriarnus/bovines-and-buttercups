...
import net.merchantpug.bovinesandbuttercups.componability.ILockdownCapability;
...
public class LockdownClientEffectExtensions implements IClientMobEffectExtensions {
    ...
    public boolean renderInventoryIcon(MobEffectInstance instance, EffectRenderingInventoryScreen<?> screen, GuiGraphics guiGraphics, int x, int y, int blitOffset) {
        Minecraft.getInstance().player.getCapability(ILockdownCapability.INSTANCE).map(cap -> cap.getLockdownMobEffects().entrySet().stream().toList()).ifPresent(list -> {
            if (!list.isEmpty()) {
