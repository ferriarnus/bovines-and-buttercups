...
public class CustomMushroomItemRendererHelper {
    public static void render(ItemStack stack, PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay, ItemDisplayContext transformType) {
        ...
        if (CustomMushroomItem.getMushroomTypeFromTag(stack, level.registryAccess()).isPresent()) {
            Optional<ResourceLocation> modelLocationWithoutVariant = BovineStatesAssociationRegistry.getItem(CustomMushroomItem.getMushroomTypeKeyFromTag(stack, level.registryAccess()), BovineBlockstateTypes.MUSHROOM, false);
