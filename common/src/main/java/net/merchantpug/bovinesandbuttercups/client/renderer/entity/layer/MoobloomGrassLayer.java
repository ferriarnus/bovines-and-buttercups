...
public class MoobloomGrassLayer<T extends Moobloom, M extends CowModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int light, Moobloom entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        if (entity.isInvisible() || !entity.getCowType().isBound() || entity.getCowType().value().configuration().backGrass().isEmpty())
            return;
        float r = 1.0F;
        float g = 1.0F;
        float b = 1.0F;

        if (entity.getCowType().value().configuration().backGrass().isPresent() && entity.getCowType().value().configuration().backGrass().get().grassTinted()) {
        ...
        this.getParentModel().renderToBuffer(poseStack, buffer.getBuffer(RenderType.entityTranslucent(entity.getCowType().value().configuration().backGrass().get().textureLocation().withPath(string -> "textures/entity/" + string + ".png"))), light, LivingEntityRenderer.getOverlayCoords(entity, 0.0F), r, g, b, 1.0F);
    }
}
