...
public class MoobloomGrassLayer<T extends Moobloom, M extends CowModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int light, Moobloom entity, float limbAngle, float limbDistance, float tickDelta, float animationProgress, float headYaw, float headPitch) {
        if (entity.isInvisible() || entity.getMoobloomType().configuration().backGrass().isEmpty()) return;
        float r = 1.0F;
        float g = 1.0F;
        float b = 1.0F;

        if (entity.getMoobloomType().configuration().backGrass().isPresent() && entity.getMoobloomType().configuration().backGrass().get().grassTinted()) {
        ...
        this.getParentModel().renderToBuffer(poseStack, buffer.getBuffer(RenderType.entityTranslucent(entity.getMoobloomType().configuration().backGrass().get().textureLocation())), light, LivingEntityRenderer.getOverlayCoords(entity, 0.0F), r, g, b, 1.0F);
    }
}
