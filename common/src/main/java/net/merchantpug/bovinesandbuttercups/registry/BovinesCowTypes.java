...
public class BovinesCowTypes {
    ...
    public static final RegistrationProvider<CowType<?>> COW_TYPE = RegistrationProvider.get(BovinesRegistryKeys.COW_TYPE, BovinesAndButtercups.MOD_ID);

    public static final Holder<CowType<MoobloomConfiguration>> MOOBLOOM_TYPE = register("moobloom", () -> new CowType<>(MoobloomConfiguration.CODEC, BovinesAndButtercups.asResource("missing_moobloom"), MoobloomConfiguration.DEFAULT));
    ...
    public static RegistrySetBuilder.RegistryBootstrap<ConfiguredCowType<?, ?>> bootstrap() {
        return bootstapContext -> BovinesRegistries.COW_TYPE_REGISTRY.entrySet().forEach(entry -> bootstapContext.register(ResourceKey.create(BovinesRegistryKeys.CONFIGURED_COW_TYPE, entry.getValue().defaultConfiguredId()), entry.getValue().defaultConfigured().get()));
    }
